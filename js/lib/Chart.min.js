...
(function() {
	...
	e.Point = e.Element.extend({
		...
		draw: function(i,colorType) {
			if (this.display) {
				var t = this.ctx,xLabelType = i;
				if(1 == xLabelType || 2 == xLabelType){
					if(3 == colorType){
						 t.beginPath(),
	               		 t.font ="normal bold 20px arial", 
	              	     t.fillStyle = "#11AEE4",
	               	     t.fillText(this.value, this.x + this.radius/2 + 1, this.y - 40,25),
	                     t.closePath()
	                     t.beginPath(),
	                     t.font ="24px", 
	                     t.fillStyle = "#979797",
	                     t.fillText("today", this.x + this.radius/2 + 3, this.y - 20,25),
	                     t.closePath(),
						 t.beginPath(),
						 t.arc(this.x, this.y, this.radius + 2, 0, 2 * Math.PI),
		       ...
		       }else if(3 == xLabelType){
		       			 if(3 == colorType || 4 == colorType){
		       			 	 t.beginPath(),
							 t.arc(this.x, this.y, this.radius + 2, 0, 2 * Math.PI),
							 t.lineWidth = this.strokeWidth,
							 t.fillStyle = "white",
	...
	e.Arc = e.Element.extend({
		...
		draw: function(t) {
			 var i = this.ctx,value = this.value,chartData = String(value).split("."),showCenter = this.showCenter,leftV = chartData[0],rightV = typeof(chartData[1]) == "undefined" ? "0" : chartData[1];
            if(showCenter){
                var fontBig = this.x * 8 / 15,fontSmall = this.x * 2 / 5,distanceX = this.x / 5,distanceY = this.x / 15,label = this.label,fontColor = "#979797";
                if("Glow" === label){
                    fontColor = "#597DBD";
                }
                i.beginPath(),
                i.font = fontBig + "px Lato", 
                i.fillStyle = fontColor,
                i.textAlign = "center",
                i.textBaseline = "middle",
                i.fillText(leftV, this.outerRadius - distanceX ,this.y),
                i.closePath(),

                i.beginPath(),
                i.font = fontSmall + "px Lato", 
                i.fillStyle = fontColor,
                i.textAlign = "center",
                i.textBaseline = "middle";
                if(rightV.length < 2 && "Glow" !== label){
                    i.fillText("." + rightV + "0", this.outerRadius + distanceX,this.y + distanceY)
                }else{
                    i.fillText("." + rightV, this.outerRadius + distanceX,this.y + distanceY)
                }
	...
	e.Scale = e.Element.extend({
		...
		buildYLabels: function() {
			...
			this.yLabelWidth = this.display && this.showLabels ? W(this.ctx, this.font, this.yLabels)  + 1: 0
		},
		...
		calculateXLabelRotation: function() {
			...
			if (this.xScalePaddingRight = s / 2 + 3, this.xScalePaddingLeft = e / 2 > this.yLabelWidth + 10 ? e / 2 : this.yLabelWidth + 10, this.xLabelRotation = 0, this.display) {
				...
				this.xLabelRotation > 0 && (this.endPoint -= Math.sin(x(this.xLabelRotation)) * o + 25)
		...
		calculateX: function(t) {
			var i = (this.xLabelRotation > 0, (this.width - 10) - (this.xScalePaddingLeft + this.xScalePaddingRight)),
		...
		draw: function(k,datasets) {
			var t = this.ctx,
			data = datasets[0].points,
			...
			function(i, e) {
				...
				if(1 == xLabelType){
					...
					if("" !== i){
					    t.fillText(new Date(i).format('d'), 6, 22);
				    }
					t.beginPath();
					var img=new Image();
					if(data[e].value == 0){
						img.src="css/images/graph_unchecked.png";
					}else{
						img.src="css/images/graph_checked.png";
					}
					t.drawImage(img,-10,-5);
					t.closePath();
					if(e == 0){
						t.beginPath(),
	               	 	t.font ="12px Lato", 
	                	...
	                	t.textAlign = o ? "right": "center",
					    t.textBaseline = o ? "middle": "top",
	                	t.fillText(new Date(i).format('MMMM'), 10,34),
	               		t.closePath()
					}
					if(e == data.length - 1){
						t.beginPath(),
	                	...
	                	t.textAlign = o ? "right": "center",
					    t.textBaseline = o ? "middle": "top",
	                	t.fillText(new Date(i).format('MMMM'), 12,34),
	               		t.closePath()
					}
				}else if(2 == xLabelType){
					if(e == 0){
					t.fillStyle = "#4A90E2"
				    }
					else if(e == data.length - 1){
	...
	i.Type.extend({
		...
		draw: function(t) {
			...
			function(t, e) {
				...
				i);
				var showCenter = this.segments[e].showCenter;
                if(showCenter){
                    var canvasGradient = this.chart.ctx.createLinearGradient(this.segments[e].x - 30 ,this.segments[e].y - 30,this.segments[e].x + 15,this.segments[e].y + 15),
                    fillColor = this.segments[e].fillColor,highlightColor = this.segments[e].highlightColor;
                    if(typeof(fillColor) == 'string'){
                    canvasGradient.addColorStop(0.7, fillColor);
                    canvasGradient.addColorStop(1, highlightColor);
                    t.fillColor = canvasGradient
                 }
          	  }
	...
	i.Type.extend({
		...
		initialize: function(t) {
			this.PointClass = i.Point.extend({
				...
				ctx: this.chart.ctx,
				inRange: function(t) {
					//console.log("t : " + t + ", this.x : " + this.x + " ,this.radius : " + this.radius + ", this.hitDetectionRadius : " + this.hitDetectionRadius);
					//console.log(Math.abs(t - this.x) <= this.hitDetectionRadius);
					return Math.abs(t - this.x) < (this.hitDetectionRadius - 10)
		...
		buildScale: function(t) {
			...
			o = {
				...
				width: this.chart.width,
				ctx: this.chart.ctx,
				xLabelType : this.options.xLabelType,
				textColor: this.options.scaleFontColor,
				fontSize: this.options.scaleFontSize,
				fontStyle: this.options.scaleFontStyle,
				fontFamily: this.options.scaleFontFamily,
		...
		draw: function(t) {
			...
			this.scale.draw(i,this.datasets),
			e.each(this.datasets,
			function(t) {
			    ...
			    var canvasGradient = s.createLinearGradient(0, 0, 620,600);
                canvasGradient.addColorStop(0.2, "#FF00AE");
                canvasGradient.addColorStop(1,"#00ADFF");
				s.strokeStyle = canvasGradient,
				s.beginPath(),
				e.each(h,
				function(t, i) {
				...
				if(pointsArr.length > 2){
				 var lastSecond = pointsArr[pointsArr.length - 2],
				 	 last = pointsArr[pointsArr.length - 1];
				 	 if(last > lastSecond){
						mapPoints[pointsArr[pointsArr.length - 1]].draw(this.scale.xLabelType,3);
				 	 }else{
				 	 	mapPoints[pointsArr[pointsArr.length - 1]].draw(this.scale.xLabelType,4);
				 	 }

				 	 if(3 == this.scale.xLabelType){
				 	 	 var second = pointsArr[1],
				 	         first = pointsArr[0];
					 	 if(first > second){
							mapPoints[pointsArr[0]].draw(this.scale.xLabelType,3);
					 	 }else{
					 	 	mapPoints[pointsArr[0]].draw(this.scale.xLabelType,4);
					 	 }
				 	 }
                 
               }
               s.closePath();
               if(this.options.dashedLine){
	               s.beginPath(),
	               s.strokeStyle = "#F3F3F3",
	               s.fillStyle = "#979797",
	               s.moveTo(this.scale.calculateX(0),this.scale.calculateY(60)),
	               s.lineTo(this.scale.width,this.scale.calculateY(60)),
 				   s.stroke(),
	               //s.dashedLineTo(this.scale.calculateX(0),this.scale.calculateY(60),this.scale.width,this.scale.calculateY(60),4),
	               s.closePath()
               }
			},
			this)
